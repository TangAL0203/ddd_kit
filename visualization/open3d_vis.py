...
class Visualizer(object):
    ...
    Args:
        ...
        intrinsics (numpy.array, shape=[4, 4]): intrinsics.
        extrinsics (numpy.array, shape=[4, 4]): extrinsics.
    ...
    def __init__(self,
        ...
        self.o3d_visualizer.create_window(
            window_name="o3d", width=self.w, height=self.h, left=50, top=50,
            visible=True)
    ...
    def show(self, save_path=None):
        ...
        if self.intrinsics is not None and self.extrinsics is not None:
            ctr = self.o3d_visualizer.get_view_control()
            init_param = ctr.convert_to_pinhole_camera_parameters()

            K = self.intrinsics[:3, :3].copy()
            K[0] /= 2
            K[1] /= 2
            K[0, 2] = self. w // 2 - 0.5
            K[1, 2] = self.h // 2 - 0.5
            ...
            init_param.extrinsic = self.extrinsics
            ctr.convert_from_pinhole_camera_parameters(init_param)

        while True:
            if not self.o3d_visualizer.poll_events():
                break
